...
(function ($) {
  ...
  jQuery.fn.extend({
    viewPicker: function (viewPickerWrapper) {
      ...
      function getViewPicker(viewType) {
        ...
        var listPicker = $(
          '<div class="view-picker__item list-view" data-view-type="list"></div>'
        );
        var gridPicker = $(
          '<div class="view-picker__item grid-view" data-view-type="grid"></div>'
        );
  ...
  Drupal.behaviors.uiDialog = {
    attach: function (context, settings) {
      if (
        $(".ui-dialog", context).css("display") == "none" ||
        $(".ui-dialog").length === 0
      ) {
        // Fallback.
